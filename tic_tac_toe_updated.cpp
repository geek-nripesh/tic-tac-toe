		...
		while(!isBoardFull(board) && winner == 'N'){
			...
			if(firstMove){
				...
				while(!spotTaken){	//check if that spot is already occupied or not	
					...
					readUserMove(&userRow, &userCol, &stepsWon); //read input from user
				...
				if(gameWon(board, 'X')){
					//cout<<"Congrats "<<name<<" !!! You won against Computer.\n";
					winner = 'X';
				}
			}
			else{
				...
				while(!spotTaken){
					if(opp){
						spotTaken = getComputerMove(board, &compRow, &compCol, userRow, userCol, difficulty);
						if(spotTaken){
							break;
						}
				...
				if(gameWon(board, 'O')){
					//cout<<"Alas ! Player "<<name<<" was defeated by computer.\n";
					winner = 'O';
				}
			}
...
void createInitialBoard(char arr[N][N]){
	cout<<endl;
	for(int i=0; i<N; i++){
		for(int j=0; j<N; j++){
			arr[i][j]=' ';
...
void printCurrentBoard(char arr[N][N]){
	for(int i =0 ; i<3 ; i++){
		cout<<"\t\t";
		for(int j=0 ; j<3 ; j++){
			if(j<2)
...
void readUserMove( int *userRow, int *userCol , int *stepsWon){
	//cout<<"Your move - Enter numbers between 1 and 3.\n";
...
int isInputValid(int a, int i, int j){
	if(a>=i && a<=j){
		return 1;
	}
	else{
...
int getComputerMove (char board[N][N], int * computerRow, int * computerCol, int userRow, int userCol, char d){ 
	if(d == 'h'){
		int winningSpot = 0; 
		...
		if (!winningSpot) {
			...
			if (blocked == 0){
				computerPlaysRandom(board, computerRow, computerCol); 
			} 
			return blocked; 
		} 
		return winningSpot;	
...
void computerPlaysRandom (char board[N][N], int * cRow, int * cCol){
	*cRow = rand()%3;
	*cCol = rand()%3;
	...
	if(board[*cRow][*cCol] == ' ')
		...
		computerPlaysRandom(board, cRow, cCol);
}

int computerPlaysToBlock(char arr[N][N], int *cRow, int *cCol){
	//case 1
...
int computerPlaysToWin(char arr[N][N], int *cRow , int *cCol ){
	//case 1
...
int gameWon(char arr[N][N], char b){
...
void displayScore(int s1, int s2, string n1, string n2="Computer"){
	cout<<"\t\tScore\n"<<n1<<" : "<<s1<<"\t"<<n2<<" : "<<s2;
}
